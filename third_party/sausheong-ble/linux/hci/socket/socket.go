...
func open(fd, id int) (*Socket, error) {
	// HCI User Channel requires exclusive access to the device, and the
	// device has to be DOWN at the time of binding. Keep the original
	// DOWN->UP->DOWN reset to give the kernel time to settle (a single
	// DOWN races and bind sometimes fails on the first invocation), but
	// ignore errors on each: newer kernels return EINVAL from HCIDEVUP,
	// and the device may already be in the requested state.
	_ = ioctl(uintptr(fd), hciDownDevice, uintptr(id))
	_ = ioctl(uintptr(fd), hciUpDevice, uintptr(id))
	_ = ioctl(uintptr(fd), hciDownDevice, uintptr(id))
