	...
	for i := 0; i < int(e.NumReports()); i++ {
		...
		case evtTypScanRsp:
			...
			for idx := h.adLast - 1; idx != h.adLast; idx-- {
				...
				// Hoist the slot to a local so a concurrent Scan() that
				// reassigns h.adHist (gap.go:31) can't race the
				// nil-check with the deref on line below.
				cur := h.adHist[idx]
				if cur == nil {
					break
				}
				if cur.Addr().String() == sr.Addr().String() {
					cur.setScanResponse(sr)
					a = cur
					break
				}
			}
