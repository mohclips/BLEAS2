...
//
// `>= w` (not `> 0`) protects against malformed advertisements where the
// UUID-list field length isn't a multiple of the UUID width. Without the
// check, d[:w] silently reads past len(d) into the slice's cap (Go allows
// this), producing a garbage UUID, then d = d[w:] panics with [w:len(d)]
// because the implicit end is len(d) < w. Drop the truncated trailer.
...
func serviceDataList(sd []ble.ServiceData, d []byte, w int) []ble.ServiceData {
	// Drop malformed fields that can't even hold the UUID prefix —
	// otherwise `make([]byte, len(d)-w)` panics with a negative length
	// and ble.UUID(d[:w]) reads garbage from the slice's cap.
	if len(d) < w {
		return sd
	}
